...
<script type="text/javascript">
	$(function () {
		...
		$("#searchBtn").keydown(function (event) {
			if(event.keyCode==13){
				$.ajax({
					url:"workbench/clue/showActivityListSSS.do",
					dataType:"json",
					type:"get",
					data:{
						"clueId":"${param.clueId}",
					...
					success:function (data) {
						var html="";
                         $.each(data,function (i,n) {
						     html+='<tr>';
							 html+='<td><input type="radio" value="'+n.id+'" name="love"/></td>';
		...
		$("#submitBtn").click(function () {
			var activityId=$("#searchBody input[name='love']:checked").val();
			//alert(activityId);
...
<body>
	...
	<div id="create-transaction2" style="position: relative; left: 40px; top: 20px; width: 80%; background-color: #F7F7F7; display: none;" >
		...
		<form method="post" action="workbench/clue/convert.do"  id="transForm">
			<%--<input type="hidden" name="owner"  value="${param.ID}">--%>
			<input type="hidden" name="clueId"  value="${param.clueId}">
			<input type="hidden" name="flag"  value="success">
		  ...
		  <div class="form-group" style="width: 400px;position: relative; left: 20px;">
			...
			<div class="form-group" style="width: 400px;position: relative; left: 20px;">
				...
				<select id="type"  class="form-control" name="type">
					<option></option>
					<gyp:forEach items="${transactionTypeList}" var="t">
						<option value="${t.value}">${t.text}</option>
					</gyp:forEach>
				</select>
			</div>
		  <div class="form-group" style="width: 400px;position: relative; left: 20px;">
		    <label for="activity">市场活动源&nbsp;&nbsp;<a href="javascript:void(0);" data-toggle="modal"  id="openSearchModal" data-target="#searchActivityModal" style="text-decoration: none;"><span class="glyphicon glyphicon-search"></span></a></label>
